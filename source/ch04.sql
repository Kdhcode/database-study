...
SELECT *, MAX(price), MIN(price) 
FROM burgers;
-- 이 둘은 동시에 쓸수 없음, 왜냐하면 집게는 그룹 단위연산이고 *는 행 단위 열람이기 때문
...
SELECT COUNT(*)
FROM transactions
WHERE msg = 'Amazon' OR msg = 'Coupang';

-- 위 쿼리를 IN 연산자(목록에 포함된 값 찾기)를 활용한 버전으로 다시 작성한다면?
SELECT COUNT(*)
FROM transactions
WHERE msg IN('Amazon', 'Coupang');
-- IN 의미: msg가 () 안에 포함이 되어 있다면 TURE
-- IN 연산자를 사용하면 횔씬 더 직관적이고 편리함
...
SELECT AVG(amount)
FROM transactions
WHERE (amount > 0) AND msg IN('Amazon', 'Google');
...
-- ② DATETIME: YYYY-MM-DD hh:mm:ss 형식으로 날짜와 시간을 나타내는 자료형
-- ③ AVG(): 평균을 계산하는 함수
-- ④ IN: 주어진 목록 값 중 하나에 해당하는지 확인해 주는 연산자
-- ⑤ DISTINCT: 중복을 제거하여 유일한 값만 남기는 키워드
