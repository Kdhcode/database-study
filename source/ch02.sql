...
/*
    ...
    2.1 데이터 CRUD
*/
-- CRUD란? DB가 제공하는 기본 동작을 데이터 관리의 가장 핵심적인 기능  
-- 생성(Create)
-- 조회(Read)
-- 수정(Update) 
-- 삭제(Delete)
...
CREATE TABLE burgers (
    ...
    -- VARCHAR(N): 믄자를 최대 N자리까지 저장(가변 길이 타입이라 저장될 수 있는 최대 길이의 제한을 알려줘야 함)
    price INTEGER, -- 가격(정수)
    gram INTEGER, -- 무게(정수)
    kcal INTEGER, -- 열량(정수)
...
-- 기본키: 레코드(row)를 대표하는 컬럼 (예: 사람의 주민들록번호, 학생의 학번, 군인의 군번, 사원의 사번)
...
-- (참고) 테이블 변경하기 
-- ALTER TABLEL: 이미 만든 테이블의 구조 변경
-- 예: 포인트 제도가 새로 생겨서 고객 테이블에 point 열을 추가 해야 할 때
...
-- burges 테이블의 name 컬럼을 더 길게 만들어야 한다면? 
...
-- 수백만, 수천만 건의 데이터가 들어 있는 거대한 테이블의 구조를 변경하는 적업은 많은 시간과 시스템 자원을 소모
-- 작업 중에는 테이블이 잠겨서 서비스가 일시적으로 멈출수 도 있음
...
-- 데이터 조회하기
-- SELECT 컬럼명1, 컬럼명2,... -- 조회할 컴럼(컬럼명 대신 *을 쓰면 모든 컬럼 조회)
-- FROM 테이블 명 
...
-- 컬럼 생략 가능? 가능하지만 테이블 정의 맞게 모든 열에 순서대로 값을 넣어야 함
-- 실무에서는 항상 컴럼을 명시 하는 방식을 권장 
INSERT INTO burgers
VALUES (2, '빅맨', 5300, 233, 583, 27);
...
-- ① CREATE TABLE 문: 테이블을 만드는 SQL 문
-- ② INTEGER: 정수형 숫자(-1, 0. 1, )를 저장하기 위한 자료형
-- ③ VARCHAR: 문자를 저장하기 위한 자료형(최대 길이 지정 가능)
-- ④ INSERT INTO 문: 테이블에 데이터를 삽입하는 SQL 문
-- ⑤ SELECT 문: 테이블의 데이터를 조회하는 SQL 문
...
-- 모든 값을 일관적으로 변경하는 것을 크라이언트에서 막고 있음 
...
-- '슈비두밥 버거'가 단종됐다면, 이를 위한 데이터 삭제 퀴리는?
DELETE FROM burgers
