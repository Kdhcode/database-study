...
-- UNSIDNED(언사인드) 제약 조건 부여 가능: 음수 값을 허용하지 않는 정수
-- 유효성 보장:  나이는 0 ~ 255 의 유효한 값만 저장
CREATE TABLE users (
    age TINYINT UNSIGNED
);

-- 안정성 보장: 재고는 음수가 될수 없다
...
-- 학생 기록부 (student_records) 테이블 생성
CREATE TABLE student_records(
    id int, -- 아이디(표준 정수)
    grade TINYINT UNSIGNED, -- 학년(부호가 없는 매우 작은 정수),0~255
    average_score FLOAT, -- 평균 점수(부동 소수점 방식의 실수)
    tuition_fee DECIMAL(10,2), -- 수업료(고정 소수점 방식의 실수), 돈 계산 관련은 정확하게, 전체 10자리, 소수점 이하 2자리
    PRIMARY KEY(id)
);
...
-- 과거에는 "길이가 고정적이만 CHAR, 가변적이면 VARCHAR" 라는 원칙이 강조되었지만,
...
-- 데이터베이스에 직접 저장하지 않고, 클라우드 스토이리자 파일 서버 등에 저장한 뒤,
-- 그 경로나 URL만 데이터 베이스에 저장하는 방식이 사실상 표준
...
-- 4. 특정 범위 필터링하기
-- 1) DETWEEN 연산자
-- 두 값 사이에 속하는지 확인 할 때 사용하는 연산자(두값을 포함하여 찾음)
SELECT *
FROM 테이블명
...
-- (참고) price 상품 가격이 10000~20000원 사이에 아닌 주문은?
SELECT *
FROM orders
...
-- 첫 글자가 'ㄱ'으로 사작하는 걸 찾으려면 유니코드 범위를 활용
-- 가(AC00) ~ 깋(ACFF)
SELECT *
FROM orders
WHERE name >= 'ㄱ' AND name < '나';
...
-- ① LIKE: 문자열이 특정 패턴과 완전히 혹은 일부와 일치하는지 확인하는 연산자
-- ② YEAR(): 입력 날짜의 연도를 반환하는 함수
-- ③ EXTRACT(): 입력받은 날짜 데이터에서 특정 필드를 추출하는 함수
-- ④ BETWEEN: 시작 값과 마지막 값을 포함해 두 값 사이의 범위에 속하는지 확인하는 연산자
-- ⑤ TIME_TO_SEC(): 입력 시간의 시, 분, 초를 '초'로 환산하는 함수
