...
VALUES
    ...
    -- IS NOT NULL: 해당 열의 값이 NULL 인 아닌, 즉 데이터가 입력된 행을 찾음 
...
-- WHERE ! protein > 25 ; --(참고) &&, ||, !는 MySQL어서만 작동
...
FROM burgers;
-- 100g당 가격을 계산하여 price/100g 이라는 별칭(alisa)으로 반환함
...
-- ① WHERE: 테이블 조회 시 필터링 조건을 작성하기 위해 사용하는 명령 
-- ② !=: 같지 않음을 비교하는 연산자
-- ③ AND: 두 조건을 동시에 만족하는 논리 연산자
-- ④ OR: 두 조건 중 하나라도 만족하는 논리 연산자
-- ⑤ NOT: 특정 조건을 만족하지 않는 논리 연산자
...
FROM students
WHERE 270 < (math + english + programming);
