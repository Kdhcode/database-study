...
-- 예: 고객 테이블은 customer_id, 상품 테이블은 product_id 와 같이 테이블 명_id 형식으로 이름을 짓는 것이 일반적인 관례
-- 1부터 시작하여 데이터가 추가될 때마다 1씨기 자동으로 증가하는 정수값(INTEGER)을 기본키로 사용하는 경우가 가장 흔함
...
-- Error: Duplicate entry 'geoblo@naver.com' for key 'users.email'

INSERT INTO users (id, email, name)
VALUES (1, 'oblo@naver.com','재');
-- Error: Duplicate entry '1' for key 'users.PRIMARY'

INSERT INTO users (email, name, age)
VALUES ('geoblos@naver.com','재',15);
-- Error: Check constraint 'users_chk_1' is violated.
SELECT * FROM users
...
CREATE TABLE products (
  ...
  dc_rate INTEGER CHACK(dc_rate >= 0 and dc_rate <= 50),       -- 할인율(0~50% 제한)
  stock INTEGER UNSIGNED,                                      -- 재고 수량(음수 불가)
  PRIMARY KEY (id)                                             -- 기본키 설정: id
);

-- (참고) Tip! 날짜와 기본깞 설정 옵션
CREATE TABLE boards (
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
);
-- 이 기능을 사용하면 행이 추가되거나 변결될 때 날짜를 편리하게 관리할 수  있음

-- Quiz
-- 1. 다음 빈칸에 들어갈 용어를 순서대로 고르세요. (예: ㄱㄴㄷㄹㅁ)
-- ① 관계: 여러 테이블에 분산 저장된 데이터가 서로 어떻게 연결돼 있는지를 정의한 개념
-- ② 기본키(PK): 테이블 내 모든 튜플을 유일하게 구분하는 칼럼 또는 칼럼의 조합
-- ③ 불변성: 한 번 설정된 값은 변경될 수 없다는 성질
-- ④ NULL: 데이터가 존재하지 않는 상태
-- ⑤ 외래키(FK): 다른 테이블의 기본키를 가리키는 칼럼으로 두 테이블 간 관계를 만드는 칼럼
...
-- (참고) 실무에서는 보통 명시적으로 COUNSTRAINT 이름을 주는 것을 권장(유지보수, 디버깅 편리)
...
-- 참조 무결성 위반 시 테스트 
INSERT INTO capitals (id, name, country_id)
VALUES (104, 'Seoul', 999); -- 999는 부모 테이블에 없는 id 
-- Error: Cannot add or update a child row: a foreign key constraint fails (relation.capitals, CONSTRAINT capitals_ibfk_1 FOREIGN KEY (country_id) REFERENCES countries (id))

DELETE FROM countries
WHERE id = 1; -- 이 나라를 참조하는 수도(자식 테이블)가 있기 때문에 삭제 불가
-- Error: Cannot delete or update a parent row: a foreign key constraint fails (relation.capitals, CONSTRAINT capitals_ibfk_1 FOREIGN KEY (country_id) REFERENCES countries (id))
-- 이 경우에는 자식 테이블에서 참조하는 데이터를 모두 삭제하고 부모 테이블에서 삭제하면 가능 
...
CREATE TABLE players (
  ...
  team_id INT, -- 소속팀 아이디, 일대다 관계에서는 UNIQUE 제약조건 x
  PRIMARY KEY (id),
...
-- 또한 외래키 컬럼을 선언할 때 UNIQUE 제약 조건을 설정한다. ( o )
...
-- ① AUTO_INCREMENT: 컬럼 값이 1씩 증가하며 자동으로 입력되도록 하는 제약 조건
-- ② NOT NULL: 컬럼에 값이 반드시 입력되도록 강제하는 제약 조건
-- ③ 중간 테이블: 두 테이블 간에 다대다 관계를 맺기 위해 필요한 테이블
-- ④ BOOLEAN: 참 또는 거짓을 저장하기 위한 자료형
-- ⑤ UNIQUE: 컬럼의 값이 중복되지 않고 고유한 값만 가지게 하는 제약 조건
