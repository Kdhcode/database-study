...
SELECT AVG(height) AS '전체 평균 키' FROM students;
...
-- 그룹화 분석: 각 성별 평균키 구하기 
-- GROUP BY: 특정 컬럼의 값이 같은 행들을 하나의 그룹으로 묶어주는 역활
...
-- 단순 GROUP BY 절만 사용하는 것은 데이터를 그룹으로 묶김나 함 (의미 없음)
SELECT gender
FROM students
GROUP BY gender;
...
FROM 테이블명
WHERE 조건
GROUP BY 그룹화_컬럼1, 그룹화_컬럼2; -- 컬럼 1로 먼저 그룹화하고, 그 안에서 다시 컬럼 2로 그룹화
...
-- - 집계된 컬럼 : 집계 함수에 사용된 컬럼(그룹 전체를 요약한 값이랑 가능)
...
-- GROUP BY 절에서 지정한 그룹화 칼럼이거나 집계 함수에 사용된 칼럼이어야 한다. ( o )
...
GROUP BY 그룹화_컬럼
HAVING 그룹_필터링_조건; -- 그룹핑된 결과에 대해 필터링하는 그룹 단위 필터링 (집계함수 사용 가능)

-- 결제 유형별 평균 결제 금액이 40,000원 이산인 데이터는?
SELECT 
	ptype AS '결체 유형', 
...
FROM payments
GROUP BY ptype
HAVING `평균 결제 금액` >= 40000; -- 주위! ''로 묶으면 문자 데이터로 인식
...
-- 2. 데이터 정렬(ORDER BY)
-- 정렬: 퀴리 결과를 오름차순 또는 내림차순으로 배열하는 것
-- ORDER BY 절을 사용하여 수행 
...
SELECT 
	...
	amount AS '결제 금액'
FROM payments
ORDER BY ptype, amount DESC; -- ptype의 오름차순 정렬 후 그 안에서 amount의 내림차순 정렬(순서 중요)

-- 실무 Tip
-- 오름차순인 경우 ASC 키워드는 생략 가능(정렬의 기본 값)
...
-- 이것은 데이터 베이스 시스템마다 정책이 다를 수 있음 (예: Oracle은 NULL을 가장 큰 값으로 취급)
...
-- LIMIT 절에 OFFSET 키워드를 추가해 일어 올 데이터의 시작 지점을 조종할 수 있음
...
-- 결제 금액 4등~6등까지 조회
SELECT *				-- 2) 모든 컬럼을 선택
FROM payments			-- 1) payments
ORDER BY amount DESC	-- 3) amount 내림차순으로 정렬한 후
LIMIT 3, 3;				-- 4) 처음 3개 행은 건너뛰고 3개행만 가져온다
...
-- 대규모 대이터에서는 커서 기반 페이지네이션이나 키셋 페이지네이션을 고려하기도 함
...
SELECT 
	u.nickname AS 주문자,
	SUM(p.amount) AS '주문 총액'		 -- 6) 특정 컬럼을 조회
FROM users u							-- 1) 사용자 테이블에
JOIN orders o ON o.user_id = u.id		-- 2) 주문테이블을 붙이고
JOIN payments p ON p.order_id = o.id	-- 3) 결제 테이블을 붙이고
GROUP BY u.nickname						-- 4) 사용자별 그룹화를 진행하여
HAVING SUM(p.amount) >= 30000;			-- 5) 그룹 필터링을 하고

-- 가장 많이 팔린 상품 TOP 3
-- 상품별 판매 수량을 집계했을 때, 가장 많이 팔린 상품 TOP3의 상품 명과 판매 수량은?
-- 판매 수량이 동일할 때는 상품명 순으로 정렬
SELECT
	p.name AS 상품명,
	SUM(od.count) AS '판매 수량'				 --8
FROM products p									-- 1
JOIN order_details od ON od.product_id = p.id	-- 2
JOIN orders o ON od.order_id = o.id				-- 3
WHERE status = '배송 완료'						 -- 4
GROUP BY p.name									-- 5
ORDER BY SUM(od.count) DESC, p.name ASC			-- 6
LIMIT 3;										-- 7
